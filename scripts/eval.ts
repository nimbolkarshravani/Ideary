...
interface ExtractionOutput {
  ...
  at_a_glance?: { effort?: string; cost?: string; fit?: string; time?: string } | null;
}
...
// Import the canonical prompt from the library
import { EXTRACTION_PROMPT } from "../lib/extraction-prompt";
const INITIAL_PROMPT = EXTRACTION_PROMPT;
...
async function main() {
  ...
  // ── Log winning prompt (no longer auto-saves to lib/extraction-prompt.ts) ──

  console.log(`\n  Winning prompt length: ${bestIteration!.prompt.length} chars (iteration ${bestIteration!.iteration})`);
  ...
  lines.push("");
  lines.push("```");
  lines.push(bestIteration!.prompt);
  lines.push("```");
